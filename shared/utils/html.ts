...
function normalizeStatusMessage(message: string | null | undefined): string {
...
function extractFallbackArticleMessage($: cheerio.CheerioAPI): string {
  const candidates = [
    normalizeStatusMessage($('.weui-msg .weui-msg__title').first().text()),
    normalizeStatusMessage($('.mesg-block').first().text()),
    normalizeStatusMessage($('title').first().text()),
  ];

  return candidates.find(Boolean) || '该内容暂时无法查看';
}

export interface ArticleHtmlInspection {
  hasJsArticle: boolean;
  hasJsContent: boolean;
  hasJsBaseContainer: boolean;
  hasRenderableJsContent: boolean;
  hasRenderableBaseContainer: boolean;
  isShellPage: boolean;
}

function hasRenderableNodeContent(rawHtml: string): boolean {
  const normalizedText = normalizeStatusMessage(rawHtml.replace(/<[^>]+>/g, ' ').replace(/&nbsp;/gi, ' '));
  if (normalizedText.length > 0) {
    return true;
  }

  return /<(img|video|iframe|svg|canvas)\b/i.test(rawHtml);
}

export function inspectArticleHtml(html: string): ArticleHtmlInspection {
  const $ = cheerio.load(html);
  const $jsArticle = $('#js_article');
  const $jsContent = $jsArticle.find('#js_content').first();
  const $jsBaseContainer = $jsArticle.find('#js_base_container').first();
  const jsContentHtml = $jsContent.html() || '';
  const jsBaseContainerHtml = $jsBaseContainer.html() || '';
  const hasRenderableJsContent = $jsContent.length > 0 && hasRenderableNodeContent(jsContentHtml);
  const hasRenderableBaseContainer = $jsBaseContainer.length > 0 && hasRenderableNodeContent(jsBaseContainerHtml);

  return {
    hasJsArticle: $jsArticle.length > 0,
    hasJsContent: $jsContent.length > 0,
    hasJsBaseContainer: $jsBaseContainer.length > 0,
    hasRenderableJsContent,
    hasRenderableBaseContainer,
    isShellPage: $jsArticle.length > 0 && !hasRenderableJsContent && $jsBaseContainer.length > 0 && !hasRenderableBaseContainer,
  };
}

function isClientRenderedUnavailablePage(html: string): boolean {
  return !/id=["']js_article["']/i.test(html)
    && !/cgiDataNew/i.test(html)
    && (/window\.cgiData\s*=/.test(html) || /private-legacy|private-[a-z]+\.m/i.test(html));
}
...
export function normalizeHtml(rawHTML: string, format: 'html' | 'text' = 'html'): string {
  ...
  if ($jsArticleContent.length === 0) {
    const fallbackMessage = extractFallbackArticleMessage($);
    if (format === 'text') {
  ...
  <body>
    <main class="article-fallback">${escapeHtml(fallbackMessage)}</main>
  </body>
  </html>`;
  }
...
export function validateHTMLContent(html: string): ['Success' | 'Deleted' | 'Exception' | 'Error', string | null] {
  ...
  if ($jsArticle.length === 1) {
    ...
    // 失败，需要进一步判断失败类型
    const msg = normalizeStatusMessage($('.weui-msg .weui-msg__title').text());
    ...
    const msg = normalizeStatusMessage($msgBlock.text());
    return ['Exception', msg];
  } else if (isClientRenderedUnavailablePage(html)) {
    return ['Exception', '该内容暂时无法查看'];
  } else {
    return ['Error', null];
  }
}
